...
Los modelos de tipo FOPDT representan dinámicamente un sistema por medio de parámetros identificables como la ganancia $K_p$, la constante de tiempo $\tau$ y el tiempo muerto $\theta$. Las ganancias proporcionales tienen una interpretación física directa al relacionar las unidades de cambio entre variables. Conjuntamente, la constante de tiempo se interpreta como la velocidad de respuesta del sistema; la cual, a su vez, se complementa con el tiempo muerto al ser el tiempo antes de que el sistema reaccione frente a un cambio. Estos parámetros juntos son una representación comúnmente utilizada en procesos de control por su interpretabilidad y simpleza, subrogando en diferentes casos problemas SISO (Single Input, Single Output) \cite{ref16}. La fórmula general del modelo es la siguiente:
...
La estructura general del modelo para un problema MISO se presenta a continuación~\cite{ref20}. En esta, $n$ representa el número de variables de entrada, $p$ el número de polos y $z$ el número de ceros, siendo estos los parámetros estructurales que definen el modelo final en tiempo continuo $s$. De esta forma, se logra relacionar la variable controlada $y$ con las variables manipuladas $u$ y modelar la dinámica del sistema. Este enfoque lineal ha sido utilizado en diversos estudios de control predictivo, tanto en generadores eléctricos como en procesos químicos complejos. Al-Khamis y Gumis et al.~\cite{ref21} presentan un estudio comparativo entre un modelo en espacio de estados y un modelo basado en funciones de transferencia para el control predictivo de un generador de inducción. Sus resultados indican que el modelo de función de transferencia reduce el esfuerzo matemático, alcanza el estado estacionario más rápidamente y, en general, presenta un mejor desempeño que el modelo en espacio de estados. Por otro lado, Zhou y Zhu et al.~\cite{ref22} proponen un método de aislamiento de fallos en procesos químicos complejos, como el \textit{Tennessee Eastman Process}. Los resultados obtenidos son prometedores, ya que logran implementar un algoritmo MPC basado en un modelo identificado mediante funciones de transferencia. Sin embargo, no se encontraron estudios que utilicen este tipo de modelo con el objetivo específico de rechazo de perturbaciones.
